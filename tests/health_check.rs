...
use rusttest::configuration::{DatabaseSettings, get_configuration};
...
async fn spawn_app() -> TestApp {
    ...
    let server = rusttest::startup::run(listener, connection_pool.clone()).expect("Failed to bind address");
...
async fn health_check_works() {
    ...
    for _ in 0..100 {
        ...
        handles.push(tokio::spawn(async move {
            let _ = client.get(format!("{}/health_check", &address)).send().await;
            Ok(())
        }));
    }
